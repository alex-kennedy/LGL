...
class Matrix {
 ...
 private:
  ...
  std::pair<row_iterator, column_iterator> fetch_entry(entry_type e1,
                                                       entry_type e2,
                                                       value_type v) {
    types.insert(e2);
 ...
 public:
  ...
  void print(std::ostream& o = std::cout) {
    ...
    for (row_iterator ii = row_begin(); ii != row_end(); ++ii) {
      ...
      unsigned ctr = 0;
      for (column_iterator jj = ii->second.begin(); jj != ii->second.end();
           ++jj) {
      AGAIN:
...
void row_normalize(Matrix& m) {
  ...
  for (typename Matrix::row_iterator ii = m.row_begin(); ii != m.row_end();
       ++ii) {
    value_type total(0);
