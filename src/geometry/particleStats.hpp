...
class ParticleStats {
 ...
 protected:
  struct Stats {
    ...
    precision sumVel_;
    precision sumTemp_;
    Stats() { sumDist_ = sumVel_ = sumTemp_ = 0; }
 ...
 public:
  ...
  ParticleStats(const ParticleStats_& ps) { ParticleStats_::copy(ps); }

  void add2Stats_dx(precision d) { stats.sumDist_ += d; }

  void count(size_type c) { particleCtr_ = c; }

  void collectStatsAtIteration(size_type ii) { iteration2CollectStats = ii; }

  size_type collectStatsAtIteration() const { return iteration2CollectStats; }
  ...
  void printStats(std::ostream& o = std::cout) const { stats.print(o); }
