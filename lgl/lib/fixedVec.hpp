...
class FixedVec {
 public:
  using prec_ = decltype(std::declval<T&>() +
                         std::declval<T&>());  // this handles atomics as well
 ...
 public:
  ...
  void copy(const FixedVec<U, dimension_>& p) {
    for (size_type ii = 0; ii < dimension_; ++ii) orig[ii] = p[ii];
  }
  ...
  void translate(prec_ x) {
    for (size_type ii = 0; ii < dimension_; ++ii) orig[ii] += x;
  }
  ...
  prec_ magnitudeSquared() const {
    prec_ mag = 0;
    for (size_type ii = 0; ii < dimension_; ++ii) mag += sqr((*this)[ii]);
    return mag;
  }

  prec_ magnitude() const { return sqrt(Vec_::magnitudeSquared()); }
  ...
  prec_ distance(const Vec_& p) const { return sqrt(Vec_::distanceSquared(p)); }

  prec_ sum() const {
    prec_ sum = 0;
    for (size_type ii = 0; ii < dimension_; ++ii) sum += orig[ii];
    return sum;
  }

  prec_ average() const { return this->sum() / static_cast<prec_>(dimension_); }

  prec_ product() const {
    prec_ product = 1;
    for (size_type ii = 0; ii < dimension_; ++ii) product *= orig[ii];
    return product;
  }

  prec_ dotProduct(const Vec_& v) const {
    prec_ product = 0;
    for (size_type ii = 0; ii < dimension_; ++ii) product += orig[ii] * v[ii];
    return product;
  }
  ...
  std::ostream& print(std::ostream& o = std::cout) const {
    o << orig[0];
    for (size_type ii = 1; ii < dimension_; ++ii) o << " " << orig[ii];
    o << '\n';
    return o;
  }
  ...
  void operator-=(const Vec_& p2) {
    for (size_type ii = 0; ii < dimension_; ++ii) orig[ii] -= p2[ii];
  }
  ...
  prec_ operator[](size_type i) const { return orig[i]; }
};
