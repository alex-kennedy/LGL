...
class ParticleContainerChaperone {
 ...
 public:
  ...
  void randomizePosRange(precision p) {
    posRange_ = p;
    randomPos_ = 1;
  }
  void randomizeVelRange(precision v) {
    velRange_ = v;
    randomVel_ = 1;
  }
 ...
 private:
  ...
  void readXin() {
    ...
    while (is >> id) {
      if (!readPos(is, pos))
        throw std::domain_error("Initial position input failed for node '" +
                                id + '\'');
...
void ParticleContainerChaperone<Particle>::initAllParticles() {
  ...
  for (size_type ii = 0; ii < nodeCount; ++ii) {
    ...
    if (file_in[_X_FILE__]) {
      ...
      if (randomPos_) {
        pc_[ii].X(PCC_::randomVec(initPos_, posRange_));
      } else {
        pc_[ii].X(initPos_);
      }
    }
