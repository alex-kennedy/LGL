...
class TimeKeeper {
 ...
 public:
  ...
  bool rangeCheck() const {
    return ((iteration_ >= minIteration_) && (iteration_ <= maxIteration_));
  }
  ...
  void operator=(const TimeKeeper& tk) { TK_::copy(tk); }

  void operator++() { TK_::increment(); }

  void operator+=(prec_ dt) { TK_::increment(dt); }
};
