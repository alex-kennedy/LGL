...
typedef Grid<Particle<n_dimensions>> Grid_t;
...
typedef ParticleStats<Particle<n_dimensions>> ParticleStats_t;
...
// The program stops and writes to file at this count. (Default value).
const int MAXITER = 250000;

// This automatically dums xcoords to file at these multiples.
const int WRITE_INTERVAL = 0;  // Off by default

// The range that make casual interactions applicable
...
// This is an initial force term for the drag. The drag probably increases with
// each time step though.
const FloatType INIT_RESISTANCE = 1.0;

// This is the scale factor for all the spring constants (Scales col1 of the
// relational data)
const FloatType SPRING_CONSTANT_FACTOR = 1.0;

// This is the size of each node. It is important to have a size since if a
// particle gets really really really close the unit vector -> infinite const
// FloatType NODE_SIZE = BEST_EQ_DISTANCE*.25;
