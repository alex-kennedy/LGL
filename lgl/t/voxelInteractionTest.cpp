...
struct Pair {
  ...
  bool operator<(const Pair& other) const { return index1 < other.index1; }
  void print(std::ostream& o = std::cout) const {
    o << "I1: " << index1 << "\tI2: " << index2 << "\tD: " << d << '\n';
  }
};
...
struct less_than_d {
  bool operator()(const Pair& p1, const Pair& p2) const { return p1.d < p2.d; }
};
...
struct Handler {
  ...
  DistanceTest dtest;
  void interaction(Node_t& n1, Node_t& n2) { dtest(n1, n2); }
};
...
void gridPrepAndInit(NodeContainer_t& nc, Grid_t& nw, prec_t voxelLength);
